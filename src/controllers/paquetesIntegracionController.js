...
// ✅ CORREGIDO: ahora es ESTRICTO para cumplir “sí o sí ENTERO”
function toIntOrNull(v) {
  if (v === null || v === undefined) return null;
  const s = String(v).trim();
  if (!/^\d+$/.test(s)) return null;          // solo enteros positivos
  const n = Number(s);
...
export async function validarDisponibilidadPaquete(req, res) {
  ...
  try {
    ...
    const personasRaw = request?.personas ?? request?.Personas ?? null;

    // ✅ CORREGIDO: personas debe ser ENTERO (y puede ser 0/null)
    const personas =
      personasRaw == null || String(personasRaw).trim() === ''
        ? null
        : toIntOrNull(personasRaw) ?? 0;

    if (!fechaInicio || personas == null) {
      return res.json({
        disponible: true,
...
export async function reservarPaquete(req, res) {
  try {
    ...
    // ✅ CORREGIDO: id_reserva debe ser ENTERO seguro
    const reservaId = toIntOrNull(r.reservaId);
    if (reservaId == null) {
      return res.status(500).json({ error: 'Error al confirmar reserva', detalle: 'reservaId inválido' });
    }

    return res.json({
      id_reserva: reservaId,       // ✅ ENTERO
      id_paquete: paqueteId,       // ✅ ENTERO
      id_hold: holdId,
      correo: email,
      payment_status: paymentStatus,
      turistas,
      _links: generarLinksReserva(baseUrl, String(reservaId), null)
    });
  } catch (err) {
...
export async function buscarDatosReserva(req, res) {
  try {
    ...
    // ✅ CORREGIDO: ahora el ID debe ser ENTERO sí o sí
    const reservaId = toIntOrNull(id);
    if (reservaId == null) {
    ...
    let uriFactura = `https://facturas.example.com/${reservaId}`;
    try {
    ...
    return res.json({
      id_reserva: reservaId, // ✅ ENTERO (pedido)
      data: r,
...
export async function emitirFacturaPaquete(req, res) {
  ...
  try {
    ...
    // ✅ CORREGIDO: normaliza valor a Number
    const valorRaw = request?.valor ?? request?.Valor ?? request?.valor_pagado ?? request?.valorPagado ?? null;
    const valor = valorRaw != null && String(valorRaw).trim() !== '' ? Number(valorRaw) : null;
    ...
    return res.json({
      ...
      valorPagado: valor != null && Number.isFinite(valor) ? valor : null,
      // ✅ CORREGIDO: fecha consistente (ISO)
